...
# Load files
with open("data/eu_stats.json", "r") as f:
    players = json.load(f)

with open("data/eu_standings.json", "r") as f:
    teams = json.load(f)
...
def main():
    ...
    # Sort teams by points (desc) then round diff (desc)
    teams.sort(key=lambda t: (int(t["points"]), int(t["rd"])), reverse=True)

    # Save back the data
    with open("data/eu_stats.json", "w") as f:
        json.dump(players, f, indent=2)

    with open("data/eu_standings.json", "w") as f:
